...
class ConcurrencyManager {
  ...
  async waitForSlot(resourceId, config, req, res) {
    ...
    // 2. 尝试获取信号量（阻塞等待）
    let acquired = false
    const timeoutMs = (queueTimeout || 30) * 1000 // 转换为毫秒，默认30秒

    try {
      logger.debug(
        `⏳ Acquiring slot for ${resourceId} (waiting: ${waiting - 1}, timeout: ${queueTimeout}s)`
      )
      ...
      if (error.name === 'TimeoutError' || error.message?.includes('timeout')) {
        ...
        logger.warn(
          `⏱️ Timeout waiting for slot: ${resourceId} (waited ${queueTimeout}s)`
        )
  ...
  _getSemaphoreInfo(resourceId, config) {
    ...
    // 获取现有实例
    let semaphoreInfo = this.semaphores.get(resourceId)
